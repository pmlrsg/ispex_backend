...
log = logging.getLogger('ispex-wlcal')

class Ispexcal(object):
    """
    An instance of Ispexcal is created for each wavelength calibration image file.
    ...
    def __init__(self,
                 ...
                 output_plots=False):
        """
        Initialize with the path to the dng image file,
          camera profile, and output directory for calibration files and optional plots
        ...
        if not os.path.exists(self.dng_path):
            raise FileNotFoundError(f"File {self.dng_path} not found.")

    def process(self):

        x_offset = self.find_spectrum_in_raw_image(self.dng_path)

        save_to_Qp = os.path.join(self.save_path, "cameras", self.camera, "wavelength_calibration_Qp.npy")
        save_to_Qm = os.path.join(self.save_path, "cameras", self.camera, "wavelength_calibration_Qm.npy")

        self.log.info(f"Reading image {self.label}")
        # read the raw image
        with rawpy_lib.imread(self.dng_path) as img:
            self.img_raw = img.raw_image.astype(np.float64)
            bayer_map = img.raw_colors
            self.img_post = img.postprocess()                 #  scaled rgb image for visualisation,
                                                              #  image dimensions are 2 time larger than raw image
            width, height = img.sizes.raw_width, img.sizes.raw_height
            self.log.info(f"Raw Image Width: {width}")
            self.log.info(f"Raw Image Height: {height}")
